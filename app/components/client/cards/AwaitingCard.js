...
const AwaitingCard = ({ data, user, key, isBidPage }) => {
  ...
  return (
    ...
    >
      ...
      {!isBidPage && (
        <Pressable
          onPress={() => navigation.navigate("bids", { jobId: jobId })}
          style={style.button(isBidPage)}
        >
          <Text style={{ fontWeight: 600, lineHeight: 25 }}>View bids</Text>
        </Pressable>
      )}
      {/* {isBidPage && (
        <View
          onPress={() => navigation.navigate("jobDetails", { jobId: data.id })}
          style={style.button(isBidPage)}
        >
